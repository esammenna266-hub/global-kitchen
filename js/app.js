/**
 * Master Application Controller & Navigation Coordinator
 */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const settings = window.wcStore.getSettings();
...
function switchTab(tabId) {
    ...
    const titles = {
        'overview': { title: 'النظرة العامة', sub: 'مرحباً بك في لوحة تحكم متجرك' },
...
function initSettingsForm() {
    ...
    form.addEventListener('submit', (e) => {
        ...
        window.wcStore.saveSettings(updated);
...
function resetToDemoMode() {
    window.wcStore.saveSettings({ store_url: '', consumer_key: '', consumer_secret: '', is_connected: false });
...
function updateConnectionUI(settings) {
    ...
    if (settings.is_connected && settings.store_url) {
        ...
        if (modeText) modeText.textContent = "وضع الديمو المحلي";
        if (urlText) urlText.textContent = "demo-store.local";
    }
}
